...
-   Headless app runs also emit `headless_metrics.csv`, a per-frame metrics/export stream containing frame index, sim time, world hash, collision count, rigid-body count, dynamic-body count, transform count, update wall time, render wall time, and total frame wall time.
