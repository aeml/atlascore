...
2.  **Run Headless Scenarios**:
    You can run the main application in headless mode. This runs the full update + render path, writes render output to `headless_output.txt`, writes per-frame metrics to `headless_metrics.csv`, writes one-row aggregate run stats to `headless_summary.csv`, and writes a one-row manifest index to `headless_manifest.csv`. That manifest now also records git commit, dirty-state, and build type so you can separate simulation regressions from "you changed the binary" noise. Use `--output-prefix=PATH_BASE` when you need multiple runs side-by-side without overwriting prior artifacts.
    ```bash
