...
namespace physics
{
    namespace
    {
        CollisionResolutionSystem::SolverSettings DefaultSolverSettings()
        {
            return {};
        }
    ...
    {
        // Use the new View API for cleaner iteration
        // Note: For parallel execution, we still need to access the raw storage or implement a ParallelView.
        // For now, we'll stick to the manual storage access for the parallel path, but clean up the logic.
        ...
        auto integrateRange = [&](size_t start, size_t end) {
            for (size_t i = start; i < end; ++i) {
                ...
                // Direct access is safe here because we are iterating the dense array of RigidBodies
                // However, Transform might not be at the same index if they aren't perfectly synced.
                // So we must use Get(id) for Transform.
    ...
    void PhysicsSystem::Update(ecs::World& world, float dt)
    {
        ...
        for (int i = 0; i < substeps; ++i)
        {
            ...
            // 6. Velocity Resolution (Friction/Restitution)
            if (!m_events.empty()) {
